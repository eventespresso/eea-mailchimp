<?php
/**
 *  Meant to convert DBs between MailChimp 2.1.0 and MC 2.2.0+.
 */
/**
...
$stages = glob( ESPRESSO_MAILCHIMP_DIR . 'db/migration_scripts/2_2_0_stages/*' );
$class_to_filepath = array();
foreach( $stages as $filepath ) {
    $matches = array();
    preg_match( '~2_2_0_stages/(.*).dmsstage.php~', $filepath, $matches );
    $class_to_filepath[$matches[1]] = $filepath;
}
//give addons a chance to autoload their stages too
$class_to_filepath = apply_filters( 'FHEE__EE_DMS_MailChimp_2_2_0_stages__autoloaded_stages', $class_to_filepath );
EEH_Autoloader::register_autoloader( $class_to_filepath );


class EE_DMS_MailChimp_2_2_0 extends EE_Data_Migration_Script_Base {

    public function __construct() {
        $this->_pretty_name = __("EE4 MailChimp data Migration to 2.2.0", "event_espresso");
        $this->_migration_stages = array(
            new EE_DMS_2_2_0_mc_options()
        );
        parent::__construct();
    }
    ...
    public function pretty_name() {
        return __("EE4 MailChimp data Migration to 2.2.0", "event_espresso");
    }
