...
class EE_MC_Interest_Categories_Form extends EE_Form_Section_Proper
{
    ...
    protected function _template_setup()
    {
        ...
        if ($this->_list_id && $this->_list_id !== '-1') {
            ...
            $event_list_group = $this->_mc_controller->mci_event_selected_interests((int) $this->_event_id);
            $user_groups      = $this->_mc_controller->mci_get_users_groups($this->_list_id);
            if (! empty($user_groups)) {
                $categories = $this->_list_categories($event_list_group, $user_groups);
            } else {
                $categories['no_interests'] =
                    new EE_Form_Section_HTML(
                        EEH_HTML::p(
                            esc_html__('No groups found for this List.', 'event_espresso'),
                            'no-groups-found-notice',
                            'important-notice'
                        )
                    );
            }
    ...
    protected function _list_categories($event_list_group, $user_groups)
    {
        ...
        foreach ($user_groups as $category) {
            ...
            switch ($category->type) {
                case 'radio':
                    $section_params['html_name']                                 =
                        'ee_mailchimp_groups[' . $category->id . ']';
                    $section_params['default']                                   =
                        (isset($selected_intr[0])) ? $selected_intr[0] : '';
                    $subsections[ $interest->id . '_' . $interest->category_id ] = new EE_Radio_Button_Input(
                        $interests,
                        $section_params
                    );
                    break;
                case 'dropdown':
                    $section_params['default']                                   =
                        (isset($selected_intr[0])) ? $selected_intr[0] : '';
                    $subsections[ $interest->id . '_' . $interest->category_id ] = new EE_Select_Input(
                        $interests,
                        $section_params
                    );
                    break;
                default:
                    $subsections[ $interest->id . '_' . $interest->category_id ] = new EE_Checkbox_Multi_Input(
                        $interests,
                        $section_params
                    );
                    break;
        ...
        foreach ($this->_all_interests as $key => $intr) {
            $subsections[] = new EE_Hidden_Input([
